  ...
  static Future<sql.Database> _connect() async {
    return sql.openDatabase(
      ...
      onCreate: (db, version) {
        ...
        db.execute("CREATE TABLE account (wallet_id INTEGER NOT NULL, account_no INT NOT NULL, name TEXT, xpub TEXT NOT NULL, PRIMARY KEY (wallet_id, account_id), FOREIGN KEY (wallet_id) REFERENCES wallet(id))");
        ...
        db.execute("CREATE TABLE txn (id INTEGER PRIMARY KEY, wallet_id INT NOT NULL, txid STRING NOT NULL, time INT NOT NULL, FOREIGN KEY (wallet_id) REFERENCES wallet(id), UNIQUE(txid, wallet_id))");
        db.execute("CREATE UNIQUE INDEX idx_txid ON txn(txid)");
